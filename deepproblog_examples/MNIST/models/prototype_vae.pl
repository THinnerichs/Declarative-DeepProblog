...
encode_decode(Image, Prototype, P) :- encode(Image, Prototype, P1), decode(Prototype, Image, P2), mul(P1, P2, P), writeln(Prototype, Image). 

encode(Image, Latent, P) :- ground(Image), encoder(Image,Latent2), lat_similar(Latent, Latent2, P).
encode(Image, Latent, 1.0) :- var(Image), writeln("Bumm", Latent), decoder(Latent, Image).
