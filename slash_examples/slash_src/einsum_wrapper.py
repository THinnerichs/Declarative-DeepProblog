    ...
    def forward(self, x, marg_idx=None, type=1):
        ...
        if type == 4:
            ...
            if marg_idx:  # If marginalisation mask is passed
                ...
                log_likelihood = super().forward(x)
                likelihood = torch.nn.functional.softmax(log_likelihood, dim=1)
                #print("P(X|C):", likelihood.shape, likelihood[0], torch.sum(likelihood[0]))
            ...
            if type == 2:
                ...
                #print("P(C|X)", posterior.shape, posterior[0])
            ...
            #JOINT
                ...
                #print("P(X,C)",(joint.shape), joint[0], torch.sum(joint[0]))
