            ...
            for i, cEqualsVi in enumerate(self.pc[ruleIdx]):
                if p != 0:     
                    if not isinstance(p,torch.Tensor) :   
                        p = torch.Tensor([p])         
                    # float() keeps entries scalar so the object-array row
                    # assignment in mvppLearn broadcasts under numpy >= 1.24
                    gradients[i] = float(gradients[i]/torch.min(torch.Tensor([1]),p+self.eps))
