...
def pad_3d_tensor(target, framework, bs, ruleNum, max_classes):
    ...
    if framework == 'torch':
        padded = torch.stack([torch.hstack((row,  torch.tensor([0] * (max_classes - len(row)), device=row.device ) ) ) for batch in target for row in batch ]).view(ruleNum, bs, max_classes)
    return padded
        ...
        for epochIdx in range(epoch):
            ...
            for data_batch, query_batch in tqdm(dataset_loader):
                ...
                try:
                    ...
                    # ragged stable-model lists cannot be concatenated as a
                    # rectangular array under numpy >= 1.24; keep them as a
                    # flat object array instead
                    model_batch_list = np.empty(
                        sum(len(s) for s in model_batch_list_splits), dtype=object)
                    model_batch_list[:] = [m for s in model_batch_list_splits for m in s]
                    #print(e)
