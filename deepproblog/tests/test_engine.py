import pytest
from problog.logic import Term, Constant, Var

from deepproblog.engines import ApproximateEngine
from deepproblog.model import Model
from deepproblog.query import Query


def test_ad():
    program = """
0.5 :: a; 0.5 :: b.
c :- a,b.
d :- a,a.
    """
    model = Model(program, [], load=False)
    engine = ApproximateEngine(model, 10, ApproximateEngine.geometric_mean)
    q = Query(Term("c"))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(0.0) == r.result[Term("c")]
    q = Query(Term("d"))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(0.5) == r.result[Term("d")]


def test_ad2():
    program = """
0.5 :: a; 0.5 :: b.
0.5 :: a; 0.5 :: b.
c :- a,b.
d :- a,a.
    """
    model = Model(program, [], load=False)
    engine = ApproximateEngine(model, 10, ApproximateEngine.geometric_mean)
    q = Query(Term("c"))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(0.5) == r.result[Term("c")]
    q = Query(Term("d"))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(0.75) == r.result[Term("d")]


def test_ad3():
    program = """
0.5 :: a(X); 0.5 :: b(X).
c :- a(1),b(1).
d :- a(1),b(2).
    """
    model = Model(program, [], load=False)
    engine = ApproximateEngine(model, 10, ApproximateEngine.geometric_mean)
    q = Query(Term("c"))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(0.0) == r.result[Term("c")]
    q = Query(Term("d"))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(0.25) == r.result[Term("d")]


def test_fact():
    program = """
0.8::d(_).
e :- d(1),d(2).
f :- d(1),e.
    """
    model = Model(program, [], load=False)
    engine = ApproximateEngine(model, 10, ApproximateEngine.geometric_mean)
    q = Query(Term("e"))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(0.64) == r.result[Term("e")]
    q = Query(Term("f"))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(0.64) == r.result[Term("f")]


def test_fact2():
    program = """
0.6::a(_).
0.5::a(_).
c :- a(1),a(2).
d :- a(1),a(1).
    """
    model = Model(program, [], load=False)
    engine = ApproximateEngine(model, 10, ApproximateEngine.geometric_mean)
    q = Query(Term("c"))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(0.64) == r.result[Term("c")]
    q = Query(Term("d"))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(0.8) == r.result[Term("d")]


def test_foreign():
    program = """
a(T,Out) :- evaluate(T, Out).
    """

    def evaluate(x):
        print("Inside evaluate", x)
        return Constant(int(x) ** 2)

    model = Model(program, [], load=False)
    engine = ApproximateEngine(model, 10, ApproximateEngine.geometric_mean)
    engine.register_foreign(evaluate, "evaluate", 1, 1)
    q = Query(Term("a", Constant(2), Var("X")))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(1.0) == r.result[Term("a", Constant(2), Constant(4))]
    q = Query(Term("a", Constant(2), Constant(4)))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(1.0) == r.result[Term("a", Constant(2), Constant(4))]
    q = Query(Term("a", Constant(2), Constant(5)))
    r = engine.query(q).evaluate(model)
    assert len(r.result) == 0


def test_foreign_text():
    program = """
a(Text,I,Word) :- writeq(a(Text,I)), ith_word(Text,I,Word).
    """

    def ith_word(text, i):
        text = text.value
        word = text.split(" ")[int(i)]
        return Constant(word)

    model = Model(program, [], load=False)
    engine = ApproximateEngine(model, 10, ApproximateEngine.geometric_mean)
    engine.register_foreign(ith_word, "ith_word", 2, 1)
    queries = [
        Query(Term("a", Constant("one two three"), Constant(x), Var("X")))
        for x in range(3)
    ]
    acs = engine.query_batch(queries)
    r = [ac.evaluate(model) for ac in acs]
    assert (
        pytest.approx(1.0)
        == r[0].result[
            Term("a", Constant("one two three"), Constant(0), Constant("one"))
        ]
    )
    assert (
        pytest.approx(1.0)
        == r[1].result[
            Term("a", Constant("one two three"), Constant(1), Constant("two"))
        ]
    )
    assert (
        pytest.approx(1.0)
        == r[2].result[
            Term("a", Constant("one two three"), Constant(2), Constant("three"))
        ]
    )


def test_assignment():
    program = """
a(X) :- X=3.
    """

    def evaluate(x):
        print("Inside evaluate", x)
        return Constant(x**2)

    model = Model(program, [], load=False)
    engine = ApproximateEngine(model, 10, ApproximateEngine.geometric_mean)
    engine.register_foreign(evaluate, "evaluate", 1, 1)
    q = Query(Term("a", Var("X")))
    r = engine.query(q).evaluate(model)
    assert pytest.approx(1.0) == r.result[Term("a", Constant(3))]
