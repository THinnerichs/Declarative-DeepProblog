...
- Generative accuracy (this script’s version, per your spec):
  For each *semantic class* c that appears in the validation set:
      - sample N images from N(e_c, GEN_SAMPLE_STD^2 I),
      - decode,
      - for each sample, find the nearest neighbour (MSE) among **validation** images,
      - predicted label = label of that nearest neighbour,
      - correct iff predicted == c.
"""
...
def generative_accuracy_single_proto(
    ...
    For each semantic class c (digit/operator) whose images are present
    in `ds`:

        - sample n images from prototype c:
              z ~ N(e_c, GEN_SAMPLE_STD^2 I)
              x_hat = dec(z)
        - for each x_hat, find nearest neighbour in ds (pixelwise MSE)
          over all validation images,
        - predicted label = label of that NN,
        - correct iff predicted == c.
    ...
    # Only evaluate over classes that actually appear in the dataset
    classes_present = torch.unique(bank_labels).tolist()
    classes_present = [int(c) for c in classes_present
                       if 0 <= int(c) < model.n_classes]
    ...
    overall_acc = correct_total / float(sample_total) if sample_total > 0 else 0.0
...
def main():
    ...
    ckpt_path = os.path.join(OUT_DIR, "protovae_single.pt")
    torch.save(
        {
            ...
            "config": {
                ...
                "n_classes": N_CLASSES,
                "img_size": IMG_SIZE,
                "train_sample_std": TRAIN_SAMPLE_STD,
                "gen_sample_std": GEN_SAMPLE_STD,
                "prior_std": PRIOR_STD,
                "beta_kl": BETA_KL,
                "alpha_cls": ALPHA_CLS,
            },
