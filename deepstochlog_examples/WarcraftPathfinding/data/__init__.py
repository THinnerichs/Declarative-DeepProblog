...
class MapTileSource:
    ...
    def __init__(self, split: str, N: int = 12, data_root: str = "data/warcraft_shortest_path"):
        ...
        self.base = os.path.join(data_root) # use for rebinned data
...
# --- tensor source that returns tiles from (split,m,i,j) ---
class TileImages:
    def __init__(self, sources_by_split):
        # dict: {"train": MapTileSource(...), "val": ..., "test": ...}
        self.sources = sources_by_split

    def __getitem__(self, item):
        # item is a tuple-like of the arguments of tile(S,M,I,J)
        # item[0] -> Split (Constant), item[1] -> M, item[2] -> I, item[3] -> J
        split = str(item[0].functor).strip('"')
        m     = int(str(item[1].functor))
        i     = int(str(item[2].functor))
        j     = int(str(item[3].functor))
        src   = self.sources[split]
        N     = src.N
        gid   = m * (N*N) + i * N + j
        return src(gid)  # (C,th,tw) float32 in [0,1]


# -------------------- datasets --------------------
class _MapExample:
    def __init__(self, split: str, map_idx: int, src: MapTileSource, N: int = 3):
        self.split = split
        self.m = map_idx
        self.N = N
        self.gold = int(rdd_shortest_cost(np.array(src.vw[map_idx])))

        toks = []
        for i in range(N):
            for j in range(N):
                toks.append(
                    Term("tensor", Term("tile",
                                        Constant(split), Constant(map_idx),
                                        Constant(i), Constant(j)))
                )
        self.tiles_list = list2term(toks)

    def to_query(self) -> Query:
        # sp_cost(map(S,M), Gold, TilesList)
        return Query(
            Term("sp_cost",
                 Term("map", Constant(self.split), Constant(self.m)),
                 Constant(self.gold),
                 self.tiles_list)
        )
